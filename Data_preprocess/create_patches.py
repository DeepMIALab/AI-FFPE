...
parser.add_argument('--downscale', default=128,type= int)
...
def stitching(file_path, downscale = down_ratio):
	start = time.time()
	...
	for i in range(total):
		...
		if current_vis_params['vis_level'] < 0:
			if len(WSI_object.level_dim) == 1:
				...
				best_level = wsi.get_best_level_for_downsample(down_ratio)
		...
		if stitch:
			...
			heatmap, stitch_time_elapsed = stitching(file_path, downscale=down_ratio)
...
seg_times, patch_times = seg_and_patch(**directories, **parameters,patch_size = args.patch_size, step_size=args.step_size, 
