...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, epoch_iter, save_result,html_update_freq):#display_current_results(self, visuals, epoch, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            for label, image in visuals.items():
                image_numpy = util.tensor2im(image)
                img_path = os.path.join(self.img_dir, 'epoch%.3d_%.3d_%s.png' % (epoch, epoch_iter, label)) #img_path = os.path.join(self.img_dir, 'epoch%.3d_%s.png' % (epoch, label))
                util.save_image(image_numpy, img_path)

            # update website
            webpage = html.HTML(self.web_dir, 'Experiment name = %s' % self.name, refresh=0)
            for n in range(epoch, 0, -1):
                for j in range(epoch_iter,0,-1*html_update_freq): 
                    header_new =  str(n)+ "_"+str( j)
                    webpage.add_header('epoch_iter [%s]' % header_new )
                    ims, txts, links = [], [], []

                    for label, image_numpy in visuals.items():
                        image_numpy = util.tensor2im(image)
                        img_path = 'epoch%.3d_%.3d_%s.png' % (n, j, label)
                        ims.append(img_path)
                        txts.append(label)
            ...
            for n in range(epoch, 0, -1):
                ...
                for label, image_numpy in visuals.items():
                    image_numpy = util.tensor2im(image)
                    img_path = 'epoch%.3d_%s.png' % (n, label) #epoch%.3d_%s.4d_%s.png,,self.display_id
                    ims.append(img_path)
                    txts.append(label)
                    links.append(img_path)
                webpage.add_images(ims, txts, links, width=self.win_size)
